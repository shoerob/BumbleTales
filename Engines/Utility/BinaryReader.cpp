...
BinaryReader::BinaryReader(const wstring &fileName)/* :
                          compressing(false),
                          compressStream(NULL),compressBuffer(NULL)*/
{
	m_file = fopen(Convert()(fileName).c_str(),"rb");
	//stream.open(Convert()(fileName).c_str(),std::ios::in | std::ios::binary);
}
...
/*void BinaryReader::CompressingMode(bool compressing,unsigned int compresedSize)
{
	if(!this->compressing && compressing)
	{
		...
		memset(compressBuffer, 0, compresedSize);
...
void BinaryReader::Read(char* data,int size)
{
	...
	fread(data,sizeof(char),size,m_file);
}
