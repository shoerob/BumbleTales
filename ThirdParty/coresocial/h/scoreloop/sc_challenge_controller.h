 ...
 *    At this stage contender decides whether the challenge
      should be open to any user or direct against a designated user.
 * -# The contender plays the game and submits the challenge and the
 *    score he obtained to the Scoreloop server.
 * -# A second Scoreloop user, the contestant, accepts the challenge
 *    and pays the challenge stake. If the challenge is open it may
 *    be accepted by any Scoreloop user. If it is a direct challenge,
 *    only the user, to whom the challenge was assigned by the contender
 *    may accept it.
 * -# The contestant plays the game and submits the score to Scoreloop.
 * -# Scoreloop compares the scores of the contender and the contestant
 *    in accordance with the game scoring schema as
 *    defined by the game developer, and declares a winner.
 * -# The winner takes the challenge prize (the combined contender
 *    and contestant stakes decreased by a challenge sink).
 *
 * Basic use:
 *
 ...
 *
 * @sa SC_Challenge, @link integration-guide-challenges Integrating Challenges @endlink,
 * @link coresocial-challenges About Challenges @endlink
 *
 */
