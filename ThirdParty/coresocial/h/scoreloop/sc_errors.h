...
/** Helper macro to save some typing when chaining operations. */
#define RET_IF_NOT_OK(retCode, op) do { DO_IF_OK(retCode, op); if (retCode != SC_OK) \
    return retCode; } while (0)
...
typedef enum SC_UserValidationError_tag {
    SC_EMAIL_ALREADY_TAKEN = (1 << 0),       /**< The E-mail is already registed */
