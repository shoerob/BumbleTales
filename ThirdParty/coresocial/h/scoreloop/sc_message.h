 ...
 * receive the message.
 *
 * Instances of this class are created by calling on the
 * SC_MessageController_CreateMessage... methods.
 *
 ...
 *
 * This call checks whether it is possible to post the message. For example, whether the user account
 * is connected to all the social networks that were referenced with
 * SC_Message_AddSocialNetworkReceiver calls.
 *
 * @param self      Instance handle
...
SC_PUBLISHED SC_Error_t
        SC_Message_IsSubmitAllowed(SC_Message_h self, SC_Bool_t* pAllowed);
